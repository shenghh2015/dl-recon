function [mse_error,df_loss,norm_grad,x] = fistav2d_bk(F, x, data, step_size, reg_param, varargin)
...
df_loss = zeros(1,10);
mse_error = zeros(1,10);
norm_grad = zeros(1,10);
counter_bk = 1;
while (~done)
    df_loss(counter_bk) = F(x, data);
    mse_error(counter_bk) = mean((x(:)-data.true_img(:)).^2);
    counter_bk = counter_bk+1;
