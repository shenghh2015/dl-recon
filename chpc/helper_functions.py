...
def Projected_PLS(model,njumps=5,version=2,theta=None,index=8018,display=False,output_folder=None,short_index='',img_shape=(256,256,1),data_dirname=None,niters=2500,cutoff=.0075,H_DIRNAME=None,linked=False,Verbose=1,TVConstraint=False,tau=838):
...
#                         print('Change in mse between projections is : ' + str(mse_change_projections))
                ...
                print('input shape: '+str(model.input_shape)+' output shape:'+str(model.output_shape))
                psnr_projection = MSE(f_true.reshape(1,ts,ts,1)[:,dif:ts-dif,dif:ts-dif,:],pred[:,md:ts2-md,md:ts2-md,:])
                before_projection = MSE(f_true.reshape(1,ts,ts,1)[:,dif:ts-dif,dif:ts-dif,:],norm_deep[:,md:ts2-md,md:ts2-md,:])
#                 if Verbose>0:
# 					print('\n****MSE before projection:'+str(before_projection)+', after projection:'+str(psnr_projection)+'****\n')
