...
num_stages= 10
nb_epochs = 1000
num_per_worker = 10   ## number of workers running on cluster
batch_size = 140
nb_train = 750
nb_test = 100
nb_val = 50
skipping_first_iteration = 0
start_index_for_loop = 9
date_str = '3.11'

## set a GPU pool for model training
# --------------------------------------------------
gpu_pool = [3,4,5,6,7]
# gpu_pool = [1,2]
num_gpus = len(gpu_pool)
env_str = ''
for i in range(len(gpu_pool)):
	env_str = env_str+str(gpu_pool[i])+','
env_str = env_str[:-1]
os.environ["CUDA_VISIBLE_DEVICES"] = env_str 
print('\n********** GPU pool configuration: "{}" **********'.format(env_str))
# --------------------------------------------------

name = 'EXP{}-'.format(date_str)

range_i = []
for i in range(num_stages):
	range_i.append(i)

for i in range_i[start_index_for_loop:]:
...
# 		testFinalf = 'datasets/v' + dataset+ '_testFinal/'
		trainf = 'datasets/v' + name+dataset+ '_train/'
		testf = 'datasets/v' + name+dataset+ '_test/'
		testFinalf = 'datasets/v' + name+dataset+ '_testFinal/'
...
# 			' --name ' + stage_name + ' --num_gpus ' + str(num_gpus) + ' --which_gpu '+str(which_gpu)+' --lr '+str(lr0)
		train_command = 'python3 train7_resnet.py --nb_filters {} --depth {} --batch_size {}'\
			' --dataset {} --nb_epochs {} --name {} --num_gpus {} --which_gpu {} --lr {}'.format(nb_filters, depth, batch_size, name+dataset, nb_epochs, stage_name, num_gpus, which_gpu, lr0)
	else:
# 		train_command = 'python3 train7_resnet.py --nb_filters ' + str(nb_filters)+  \
# 			' --depth ' + str(depth) + ' --batch_size ' + str(batch_size) + ' --dataset ' + name + str(i-1) + ' --nb_epochs ' + str(nb_epochs) +\
# 			' --name ' + stage_name + ' --AD True' + ' --num_gpus ' + str(num_gpus)+' --which_gpu '+str(which_gpu) +' --lr '+str(lr1)
		train_command = 'python3 train7_resnet.py --nb_filters {} --depth {} --batch_size {}'\
			' --dataset {} --nb_epochs {} --name {} --AD True --num_gpus {} --which_gpu {} --lr {}'.format(nb_filters, depth, batch_size, name + str(i-1), nb_epochs, stage_name, num_gpus, which_gpu, lr1)
	# train
	pretrained_model_addition = ''
	if i>0:
		pretrained_model_addition = ' --pretrained_model ' + mf + 'stage' + str(i-1) + '_model/best_simple'

	if not (skipping_first_iteration!=0 and i==range_i[start_index_for_loop]):
		print('####### Pretraining #######')
...
# 			f.write('scp ' + save_dir + '$PBS_ARRAYID.pkl bmkelly@turing.seas.wustl.edu:/home/bmkelly/dl-limitedview-prior/' + AD_folder)
	...
	def move_and_save(this_run=i,new_folder=new_folder,old_folder=old_folder,dataset_folder=dataset_dict[dataset],stage_name=stage_name,AD_folder=AD_folder,move_dataset=args.move_dataset):
		...
		os.system('ssh shenghuahe@dtn01.chpc.wustl.edu "mkdir /scratch/shenghuahe/models/' + stage_name + '"')
		move_over_str = 'scp -r {}* shenghuahe@dtn01.chpc.wustl.edu:/scratch/shenghuahe/models/{}'.format(new_folder,stage_name)
# 		move_over_str = 'scp -r ' + new_folder + ' shenghuahe@dtn01.chpc.wustl.edu:/scratch/shenghuahe/models/' + stage_name+'/'
		os.system(move_over_str)
