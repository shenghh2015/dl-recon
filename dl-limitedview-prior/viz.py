...
def run_via_python_call(model,layer,num_filters=16,size=128,iterations=20,input_img=None, input_dimension=(256,256,1),average_pixel_intensity=1,create_model_fnc = None):
	img_width, img_height = size, size
    filter_indexes = range(0, num_filters)

    input_placeholder = K.placeholder((1, img_width, img_height, input_dimension(2)))
    first_layer = ZeroPadding2D((1, 1), input_shape=(img_width, img_height, input_dimension(2)))
    first_layer.input = input_placeholder


    model = create_model_fnc(first_layer)
    model = load_model_weights(model, args.weights_path)
    layer = get_output_layer(model, args.layer)

    if img is None:
        # we start from a gray image with some random noise
        init_img = np.random.random((1, img_width, img_height,input_dimension(2))) * average_pixel_intensity*.2 + average_pixel_intensity
    else:
    ...
    for i, index in enumerate(filter_indexes):
        vizualizations[i] = visualize_filter(init_img, index, input_placeholder, iterations)
